...
public class StepPagerStrip extends View {
    ...
    private final int mPrevTabColor;
    private final int mSelectedTabColor;
    private final int mSelectedLastTabColor;
    private final int mNextTabColor;
    ...
    public StepPagerStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPrevTabColor = res.getColor(R.color.step_pager_previous_tab_color);
        mSelectedTabColor = res.getColor(R.color.step_pager_selected_tab_color);
        mSelectedLastTabColor = res.getColor(R.color.step_pager_selected_last_tab_color);
        mNextTabColor = res.getColor(R.color.step_pager_next_tab_color);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mPageCount; i++) {
            ...
            setColorForPage(mPaint, i);
            canvas.drawRect(mTempRectF, mPaint);
        }
    }

    /**
     * Sets the color for the paint based on the page number
     *
     * @param paint      The Paint object used to draw the tabs
     * @param pageNumber The page number for which to draw the tab
     */
    private void setColorForPage(final Paint paint, final int pageNumber) {
        paint.setColor(pageNumber < mCurrentPage
                ? mPrevTabColor
                : (pageNumber > mCurrentPage
                ? mNextTabColor
                : (pageNumber == mPageCount - 1
                ? mSelectedLastTabColor
                : mSelectedTabColor)));
    }
